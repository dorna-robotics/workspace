...
* **`fuse_in` is the inbound side.** `fuse` says whether a station's
  ...
  two things: the tail the previous verb left held is flushed (and its
  pending recording cancelled) before the hop is planned, so the
  previous exit stops classically; and the station's own travel is not
  folded into its approach — the hop lands on the approach's first
  point and stops, and the approach group then runs as one chain by
  itself (grouping is continuity, the seam into the group is not). Why

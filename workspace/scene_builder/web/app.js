...
function _openTargetObjectPickPanel_legacy(childName) {
  const th = _sbPanelTheme();
  closeTargetObjectPickPanel();
  const panel = document.createElement("div");
  panel.id = "targetObjectPickPanel";
  panel.style.position = "fixed";
  panel.style.right = "340px";
  panel.style.top = "18px";
  panel.style.width = "360px";
  ...
  const title = document.createElement("div");
  title.textContent = "Choose target object";
  title.style.fontWeight = "800";
  title.style.fontSize = "16px";
  title.style.marginBottom = "6px";
  panel.appendChild(title);
  ...
  const sub = document.createElement("div");
  sub.textContent = "Pick what to attach to (you can also click a target object in 3D).";
  sub.style.fontSize = "12px";
  sub.style.opacity = "0.75";
  sub.style.marginBottom = "10px";
  panel.appendChild(sub);
  ...
  function getCandidates() {
    ...
    const out = names.filter(n => n && !blocked.has(n) && objectsByName.get(n));
  ...
  function render() {
    ...
    if (!shown.length) {
      const none = document.createElement("div");
      none.textContent = "No matching objects.";
      none.style.padding = "10px";
      none.style.fontSize = "12px";
      none.style.opacity = "0.7";
      list.appendChild(none);
    ...
    for (const n of shown) {
      ...
      item.onclick = () => {
        try {
          window.builderState.targetName = n;
          window.builderState.mode = "PICK_TARGET_ANCHOR";
          closeTargetObjectPickPanel();
          openTargetAnchorMenu(n);
          showToast("Pick a target anchor (click in 3D or use the list)." );
        } catch(e) { console.error(e); }
      };
      list.appendChild(item);

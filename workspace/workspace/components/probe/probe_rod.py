...
@register("probe_rod")
class ProbeRod(Probe):
    # Straight-down calibration rod. The rod-tip position + Rx(180)
    # straight-down orientation live once in the DEFAULTS "tcp" below, which is
    # only the BASE for the tcp_<deg> yaw family: __init__ builds tcp_0..tcp_180
    # from it (each = the base spun <deg> about Z) and then drops the plain
    # "tcp". tcp_0 (same orientation) is the canonical anchor and the default
    # calibration anchor; pick a yaw at calibration time with
    # recipe.calibrate(tool_anchor="tcp_90"). Mesh: static/CAD/probe_rod.glb.
    TCP_YAWS = [0, 45, 90, 135, 180, -45, -90, -135]

    DEFAULTS = dict(
        anchors={"body": {"center": [0, 0, 0, 0, 0, 0], "tcp": [0, 0, 58.97, 180, 0, 0], "tip": [0, 0, 58.97, 180, 0, 0]}},
        collision_box=
            {"body": [
                {"pose": [0, 0.0, ((58.97)/2), 0.0, 0.0, 0.0], "scale": [43, 43.0, 58.97]},  # [xyzabc] , [lx,ly,lz]
            ]},

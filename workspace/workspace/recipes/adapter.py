...
class Adapter(Plate):
    ...
    def pick_from(self, index=None, container=None, offset=None, approach=True, exit=True, output=True, **kwargs):
        ...
        if approach:
            ...
            approach_path = [[10, 0, max(height_load,height_container) + self.padding, 0, 0, 0], 
                            [10, 0, height_load+height_tool+self.gap, 0, 0, 0],
                            [10, 0, height_load, 0, 0, 0]]
        # exit
        exit_path = []
        if exit:
            exit_path = [[0, 0, height_container+self.gap, 0, 0, 0], 
                        [0, 0, max(height_load,height_container)+self.padding, 0, 0, 0]]
    ...
    def place_in(self, index=None, container=None, offset=None, approach=True, exit=True, output=True, load_anchor="center", **kwargs):
        ...
        approach_path = []
        if approach:
            approach_path = [[0, 0, max(height_load, height_container)+self.padding, 0, 0, 0], 
                            [0, 0, height_container+self.gap, 0, 0, 0]]

        # exit
        exit_path = []
        if exit:
            exit_path = [[10, 0, height_load, 0, 0, 0], [0, 0, max(height_load, height_container)+self.padding, 0, 0, 0]]

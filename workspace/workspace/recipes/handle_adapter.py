...
class HandleAdapter:
    def __init__(self, workspace, core, 
        ...
        **kwargs
        ...
        J,C = core.IK(
                    ...
                    target_offset=[-10, 0, self.height, 180, 0, 0], 
    ...
    def pick_from(self, index="front", container=None, approach=True, exit=True, **kwargs):        
        # index
        ...
        solid_child = util.solid_attached_to_anchor(container.assembly[self.solid_name], "place")
        ...
        approach_path = []
        if approach:
            clear = np.linalg.norm(np.array(container.assembly[self.solid_name].pose("top")[0:3]) - np.array(container.assembly[self.solid_name].pose("place")[0:3]))
            approach_path = [[-10, 0, self.height, 180, 0, 0], [-10, 0, clear+12, 180, 0, 0], [-10, 0, 0, 180, 0, 0]]

        # exit
        exit_path = []
        if exit:
            exit_path = [[0, 0, self.height, 180, 0, 0]]
    ...
    def place_in(self, index="front", container=None, offset=None, approach=True, exit=True, **kwargs):
        # index
        index = index or self.anchor
        container = container or self.container
        ...
        # we check if there is an item in the index
        if util.solid_attached_to_anchor(container.assembly[self.solid_name], "place") is not None:
        ...
        if approach:
            clear = np.linalg.norm(np.array(container.assembly[self.solid_name].pose("top")[0:3]) - np.array(container.assembly[self.solid_name].pose("place")[0:3]))
            approach_path = [[0, 0, self.height, 180, 0, 0], [0, 0, clear+2, 180, 0, 0]]

        # exit
        exit_path = []
        if exit:
            exit_path = [[-10, 0, 0, 180, 0, 0], [0, 0, self.height, 180, 0, 0]]
        ...
        return util.touch(
            ...
            target_solid = container.assembly[self.solid_name],
            target_anchor = index, 
            target_offset = offset or [0, 0, 0, 180, 0, 0],
            ...
            actions= [],
            sleep= 0.1,
            attach = [component_parent.assembly[next(iter(component_parent.assembly))], {"parent": container.assembly[self.solid_name], "parent_anchor":"place", "child_anchor":"center", "offset": [0, 0, 0, 0, 0, 0], "offset_frame": "child"}],
            exit_tool = {"solid": tool.assembly[next(iter(tool.assembly))], "anchor": "tcp", "offset":[0, 0, 0, 0, 0, 0]},
            exit_path = exit_path,
            motion = self.motion,
            base_distance = self.base_distance, 

...
def create_recipes(workspace, core, speed_scale=0.5):    
    return {
        ...
        "waste_bin": PipettingSite(workspace, core, workspace.components["adapter_sbs_1"], base_distance=250, speed_scale=speed_scale),
        "tip_rack": PipettingSite(workspace, core, workspace.components["adapter_sbs_0"], base_distance=250, speed_scale=speed_scale),
        "falcon_pipepette": PipettingSite(workspace, core, workspace.components["adapter_falcon"], base_distance=200, speed_scale=speed_scale),
        
        "falcon_rack": Rack(workspace, core, workspace.components["adapter_falcon"], base_distance=200, speed_scale=speed_scale),

        "decapper": Decapper(workspace, core, workspace.components["decapper"], base_distance=200, speed_scale=speed_scale),

        "printer": Printer(workspace=workspace, core=core, component=workspace.components["printer"], speed_scale=speed_scale),

        "inspector": FixedInspector(workspace, core, component=workspace.components["vision_station"], detection_preset={}, speed_scale=speed_scale),
...
def workflow_fn(*, workspace, core):
    ...
    # ------------------------------------------------------------
    rcp = create_recipes(workspace, core, speed_scale=speed_scale)
    ...
    for tip_index in tip_list:
        ...
        for index in range(num_processed_tube):
            ...
            rcp["printer"].place(exit=False, place_z_offset=printer_place_z_offset)
            ...
            rcp["falcon_rack"].place(
                tube_index,
                place_z_offset=falcon_rack_place_z_offset,
        ...
        for index in range(num_processed_tube):
            ...
            rcp["falcon_rack"].place(
                ...
                offset=cap_offset,
                two_step_approach=True,
                place_z_offset=cap_place_z_offset,
            )

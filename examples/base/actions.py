"""base protocol — Start → Park, the canonical bookends.

The seed of every project: no per-item actions, no tool, no devices.
The workflow runs Start (motor on + home the rail — fatal "killed"
outcome if homing fails) and Park (move to park pose), then ends.
Copy this folder to start a new project and add per-item actions
between the bookends; any other example is a template for those.
"""

...
- **The outcomes** — the ways the phase can end for an item, each an
  ...
  and one that asserts the platform's `removed(item)` — the item leaves
...
| `weighed_2` | `home_2(t)` | `home_2`: back in its own rack slot, shaken and weighed twice. Inside: seated on the shaker (`on_shaker`, seat `t mod 4`, a `seat_free` capacity fact), its bank shaken (`shaken`, one cycle for the four of a load), back in its slot (`unloaded` — pass 2's gate), then weighed as in pass 1. ONE phase whose `cycle` says: bank 2 shakes while bank 1 is weighed | `weight_2_g` in `Weigh2`; `status` at Park, from the facts | every tube has `home_1` |
...
- **Stages that overlap across banks share a phase, and the phase
  declares the order.** A barrier is a wait for everyone; the shake
  and the second weighing are one row, and `Weighed2.cycle` says bank
  1 is weighed while bank 2 shakes. The shake's `pre` spans the bank;
  the phase's `route` lists it once (bt-framework-guide §13 "The
  cycle").
